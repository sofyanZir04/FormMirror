...
// 🔹 Define your event shape based on the 'form_events' table in Supabase
type FormEvent = {
  session_id: string
  project_id: string
  field_name?: string
  event_type: string // You can tighten this to a union if all values are known
  duration?: number
  created_at: string
  // Add other fields if your table includes them (e.g., id, user_id, etc.)
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Fetch with timeout, but avoid 'as any'
    ...
    let data: FormEvent[] | null = null
    let error: unknown = null

    try {
      const result = await Promise.race([fetchPromise, timeoutPromise])
      data = result.data as FormEvent[] | null
      error = result.error
    } catch (err) {
      error = err
    }
